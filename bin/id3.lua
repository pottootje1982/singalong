...
local function convertUnicode(str)
  local unicodeTag = str:match(string.format('^%c%c', 255, 254))
  if unicodeTag then
    local chars = {string.byte(str, 1, #str)}
    -- remove unicode tag
    table.remove(chars, 1)
    table.remove(chars, 1)
    chars = table.ifilter(chars, function(i,v) return v ~= 0 end)
...
local function textFrame(name)
  return function (reader, info, frameSize)
    ...
    info[name] = convertUnicode(info[name])
  end
end
